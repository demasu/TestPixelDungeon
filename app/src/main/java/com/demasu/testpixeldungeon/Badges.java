...
import com.demasu.testpixeldungeon.actors.mobs.Senior;
import com.demasu.testpixeldungeon.actors.mobs.Shielded;
...
public class Badges {
    ...
    public static void loadGlobal () {
        if ( global == null ) {
            try {
                InputStream input = Game.getInstance().openFileInput( BADGES_FILE );
    ...
    public static void saveGlobal () {
        ...
        if ( saveNeeded ) {
            ...
            try {
                OutputStream output = Game.getInstance().openFileOutput( BADGES_FILE, Game.MODE_PRIVATE );
    ...
    public static void validateLevelReached () {
        ...
        if ( !local.contains( Badge.LEVEL_REACHED_1 ) && Dungeon.getHero().getLvl() >= 6 ) {
            badge = Badge.LEVEL_REACHED_1;
            local.add( badge );
        }
        if ( !local.contains( Badge.LEVEL_REACHED_2 ) && Dungeon.getHero().getLvl() >= 12 ) {
            badge = Badge.LEVEL_REACHED_2;
            local.add( badge );
        }
        if ( !local.contains( Badge.LEVEL_REACHED_3 ) && Dungeon.getHero().getLvl() >= 18 ) {
            badge = Badge.LEVEL_REACHED_3;
            local.add( badge );
        }
        if ( !local.contains( Badge.LEVEL_REACHED_4 ) && Dungeon.getHero().getLvl() >= 24 ) {
    ...
    public static void validateBossSlain () {
        ...
        if ( badge != null ) {
            ...
            if ( badge == Badge.BOSS_SLAIN_1 ) {
                switch ( Dungeon.getHero().getHeroClass() ) {
                    case WARRIOR:
                ...
                switch ( Dungeon.getHero().subClass ) {
                    case GLADIATOR:
