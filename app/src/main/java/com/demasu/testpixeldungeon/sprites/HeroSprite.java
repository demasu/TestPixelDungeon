...
public class HeroSprite extends CharSprite {
    ...
    public HeroSprite () {
        ...
        link( Dungeon.getHero() );

        texture( Dungeon.getHero().getHeroClass().spritesheet() );
        updateArmor();
    ...
    public static Image avatar ( HeroClass cl, int armorTier ) {
        ...
        RectF frame = avatar.getTexture().uvRect( 1, 0, FRAME_WIDTH, FRAME_HEIGHT );
    ...
    public boolean sprint ( boolean on ) {
        run.setDelay( on ? 0.625f / RUN_FRAMERATE : 1f / RUN_FRAMERATE );
        return on;
    }
}
