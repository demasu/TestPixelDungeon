...
public class Tengu extends Mob {
    ...
    public void die ( Object cause ) {
        ...
        if ( !Badges.isUnlocked( badgeToCheck ) || Dungeon.getHero().subClass != HeroSubClass.NONE ) {
            Dungeon.getLevel().drop( new TomeOfMastery(), pos ).sprite.drop();
        }

        Dungeon.getLevel().drop( new ScrollOfFrostLevel(), pos ).sprite.drop();
        GameScene.bossSlain();
    ...
    private void jump () {
        ...
        for ( int i = 0; i < 4; i++ ) {
            ...
            if ( Dungeon.getLevel().map[trapPos] == Terrain.INACTIVE_TRAP ) {
    ...
    public void notice () {
        super.notice();
        yell( "Gotcha, " + Dungeon.getHero().getHeroClass().title() + "!" );
    }
