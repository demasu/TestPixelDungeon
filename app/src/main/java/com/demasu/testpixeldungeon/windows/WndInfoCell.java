...
public class WndInfoCell extends Window {
    ...
    public WndInfoCell ( int cell ) {
        ...
        int tile = Dungeon.getLevel().map[cell];
        if ( Level.water[cell] ) {
            tile = Terrain.WATER;
        ...
        if ( tile == Terrain.WATER ) {
            Image water = new Image( Dungeon.getLevel().waterTex() );
            water.frame( 0, 0, DungeonTilemap.SIZE, DungeonTilemap.SIZE );
            titlebar.icon( water );
        } else {
            titlebar.icon( DungeonTilemap.tile( tile ) );
        }
        titlebar.label( Dungeon.getLevel().tileName( tile ) );
        ...
        StringBuilder desc = new StringBuilder( Dungeon.getLevel().tileDesc( tile ) );
        ...
        resize( WIDTH, (int) ( info.getY() + info.height() ) );
    }
}
