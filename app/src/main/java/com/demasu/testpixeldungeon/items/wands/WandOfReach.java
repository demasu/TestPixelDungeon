...
import com.demasu.testpixeldungeon.effects.Swap;
...
public class WandOfReach extends Wand {
    ...
    protected void onZap ( int cell ) {
        ...
        for ( int i = 1; i < reach; i++ ) {
            ...
            int before = Dungeon.getLevel().map[c];
            ...
            Heap heap = Dungeon.getLevel().heaps.get( c );
            if ( heap != null ) {
                switch ( heap.type ) {
                    case HEAP:
                        transport( heap );
            ...
            mapUpdated = mapUpdated || Dungeon.getLevel().map[c] != before;
        }
    ...
    private void transport ( Heap heap ) {
        ...
        if ( item.doPickUp( curUser ) ) {
            ...
            Dungeon.getLevel().drop( item, curUser.pos ).sprite.drop();
        }
    }

    protected void fx ( int cell, Callback callback ) {
        MagicMissile.force( curUser.sprite.getParent(), curUser.pos, cell, callback );
