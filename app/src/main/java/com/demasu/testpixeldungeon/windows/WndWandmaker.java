...
public class WndWandmaker extends WndQuest {
    ...
    protected void onSelect ( int index ) {
        ...
        questItem.detach( Dungeon.getHero().belongings.backpack );

        Item reward = index == 0 ? Wandmaker.Quest.wand1 : Wandmaker.Quest.wand2;
        reward.identify();
        if ( reward.doPickUp( Dungeon.getHero() ) ) {
            GLog.i( Hero.TXT_YOU_NOW_HAVE, reward.name() );
        } else {
            Dungeon.getLevel().drop( reward, wandmaker.pos ).sprite.drop();
        }

        wandmaker.yell( Utils.format( TXT_FARAWELL, Dungeon.getHero().className() ) );
        wandmaker.destroy();
